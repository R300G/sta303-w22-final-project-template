...
To identify the root cause for Mingar's some device performing poorly on sleep scores, Simplicity decides to use a Poisson generalized linear mixed model. The first reason to select such model is due to the nature of the variable we are to study. Flags is a count variable that records the number of red flags of device malfunction over the period of user's sleep time, thus falls under Poisson distribution. Due to the collection method of the data, the data records large amount of observations of sleep from each users, thus it is appropriate to include customer id as random effect, which makes the model a linear mixed model. From previous data discovery, we learned that flags and sleep duration have a positive linear relation, more flags can potentially appears as sleep last longer. To standardized number of flags within the same period of time, log sleep duration is selected to be the offset of the model.
