...
```
```{r, echo = FALSE ,fig.width=8, fig.height=4}
duration_sex_byCust <- sleep_data %>% 
  mutate(sex = if_else(is.na(sex), "unkown", sex))%>%
  group_by(cust_id) %>% 
  summarise(duration_mean = mean(duration, na.rm = T),sex) %>% ungroup() 

duration_sex_byCust %>% 
  ggplot(aes(x = sex, duration_mean)) + 
  geom_boxplot(fill = "steel blue")
```
```{r, echo = FALSE ,fig.width=8, fig.height=4}
...
```
```{r, echo = FALSE ,fig.width=8, fig.height=4}
duration_population_byCust <- sleep_data %>% 
  group_by(cust_id) %>% 
  summarise(duration_mean = mean(duration, na.rm = T), Population) %>% ungroup() 

duration_population_byCust %>% 
  ggplot(aes(x = Population, duration_mean)) +
